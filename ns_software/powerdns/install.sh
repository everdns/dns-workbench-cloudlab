...
sudo cp /local/repository/ns_software/powerdns/named.conf /etc/powerdns/named.conf
if [ "$MULTIPLE_IFACE" = true ]; then
    sudo cp /local/repository/ns_software/powerdns/pdns2.conf /etc/powerdns/pdns.conf
else
    sudo cp /local/repository/ns_software/powerdns/pdns.conf /etc/powerdns/pdns.conf
fi
